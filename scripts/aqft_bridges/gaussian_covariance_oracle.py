...
def grid_K(L, m):
    ...
    for i in range(L):
        for j in range(L):
            if i+1 < L:
                K[idx(i, j), idx(i+1, j)] = K[idx(i+1, j), idx(i, j)] = -1
            if j+1 < L:
                K[idx(i, j), idx(i, j+1)] = K[idx(i, j+1), idx(i, j)] = -1
    return K
...
def build_certificate(verbose=False):
    ...
    for m in [0.5, 1.0, 2.0]:
        for N in [120, 240]:
            K = chain_K(N, m)
            c = float(np.linalg.eigvalsh(K)[0])
            S = m*m + 4.0
    ...
    for m in [1e-2, 1e-4]:
        N = 240
        c = float(np.linalg.eigvalsh(chain_K(N, m))[0])
        theta = np.log(1 + c/(m*m+4))
        q = np.arccosh(1 + m*m/2)
        if verbose:
...
def write_certificate(cert, output):
...
def main(argv=None):
    ...
    cert = build_certificate(verbose=True)
    write_certificate(cert, args.output)
